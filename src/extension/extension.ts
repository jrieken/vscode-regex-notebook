...
const kernel = new class implements vscode.NotebookKernel {
	...
	async executeCell(document: vscode.NotebookDocument, cell: vscode.NotebookCell): Promise<void> {
		...
		edit.replaceNotebookCellOutput(document.uri, cell.index, [new vscode.NotebookCellOutput([output])]);
		edit.replaceNotebookCellMetadata(document.uri, cell.index, { executionOrder: this._execOrderPool++ });
